...
import com.bezgrebelny.imgurgalleryapp.core.common.extentions.getApiResult
...
class GalleryRepositoryImpl(
    ...
    override suspend fun getGallery(page: Int) =
        getApiResult {
            api.getGallery(page = page).data.map { galleryModelMapper.map(it) }
        }

    override suspend fun getGalleryAlbum(id: String) =
        getApiResult {
            galleryAlbumMapper.map(api.getGalleryAlbum(id = id).data)
        }

    override suspend fun getGalleryImage(id: String) =
        getApiResult {
            galleryImageModelMapper.map(api.getGalleryImage(id = id).data)
        }

    override suspend fun getGalleryComments(id: String) =
        getApiResult {
